...
const resend = new Resend('re_ZSrREbKC_EDRhdaLxp3rE68Ybjzm3pdqT'); 
...
export const loginUsuario = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const codigoVerificacion: string = generarCodigoVerificacion();

        // Actualizar el código de verificación en la base de datos
        await actualizarCodigoVerificacion(usuario.correo, codigoVerificacion);

        // Enviar el código de verificación al correo del usuario usando Resend
        const { data, error } = await resend.emails.send({
            from: 'Investra <investra@resend.dev>',
            to: [correo],
            subject: 'Verificación de tu cuenta en Investra',
            html: `
            <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto;">
                ...
                <main style="padding: 20px; background-color: #f9f9f9;">
                    ...
                    <p>Gracias por iniciar sesión en <strong>Investra</strong>. Para completar tu proceso de inicio de sesión y verificar tu cuenta, por favor utiliza el siguiente código de verificación:</p>
