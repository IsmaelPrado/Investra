...
import { retirarFondos } from "../../services/transactionService";
...
const TransactionModal: React.FC<TransactionModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Validar que la cantidad sea mayor a 0 y menor que el saldo del usuario
    if (amount <= 0) {
  ...
  if (amount > userBalance) {
      showToast("El monto no puede ser mayor que el saldo disponible. Saldo disponible para retirar: $" + userBalance, "warning");
      return;
  }
...
if (paymentMethod === "3") {
  ...
  try {
    ...
    return; // Salir de la función si hay un error
        ...
        const result = await retirarFondos(transactionDetails);
...
if (userEmail && selectedTransferencia) {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-gray-900 bg-opacity-80 flex items-center justify-center z-50 max-h-[60vh] overflow-y-auto">
        <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-3xl text-white">
          <h2 className="text-2xl font-bold mb-4 text-gray-200">Retirar Fondos</h2>
          ...
          <div className="max-h-[60vh] overflow-y-auto">
            <form onSubmit={handleSubmit}>
              ...
              <AmountInput amount={amount} onAmountChange={handleAmountChange} text={'Cantidad a Retirar:'}/>
    ...
    text={"¿Estás seguro de que deseas retirar desde el método de pago seleccionado?"}
/>
