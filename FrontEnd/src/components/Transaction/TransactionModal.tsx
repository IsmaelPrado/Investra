...
import { getPaymentMethods, PaymentMethod } from '../../services/paymentMethodService';
...
const TransactionModal: React.FC<TransactionModalProps> = ({ userBalance, onClose }) => {
    ...
    const handleConfirm = async () => {
        ...
        setIsLoadingModalOpen(true); // Mostrar el modal de carga
        ...
        try {
            ...
            // Esperar 3 segundos (simulando la transacción)
            await new Promise(resolve => setTimeout(resolve, 3000)); // Modal de carga visible durante este tiempo

            // Mostrar el toast de éxito
            showToast('Transacción realizada exitosamente!', 'success');
        } catch (error: any) {
            showToast(error.message, 'error');
        } finally {
            setIsLoadingModalOpen(false); // Cerrar el modal de carga
        }
    };
        ...
        <div className="fixed inset-0 bg-gray-900 bg-opacity-80 flex items-center justify-center z-50">
            <div className="bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-md text-white">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <label htmlFor="paymentMethod" className="block text-gray-400 mb-2">Cantidad a depositar en tu cuenta Investra:</label>
                    <div className="mb-4 flex items-center">
                        
                        <span className="material-icons -outlined mr-2">attach_money</span>
                        <input
                            type="text"
                            value={amount ? new Intl.NumberFormat('es-MX', { style: 'currency', currency: 'MXN' }).format(amount) : '$0.00'}
